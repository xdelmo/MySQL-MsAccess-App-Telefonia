...
/*!50001 VIEW `privati` AS select `c`.`ID` AS `ID`,`p`.`Nome_Cliente` AS `Nome_Cliente`,`p`.`Cognome` AS `Cognome`,`p`.`Data_Nascita` AS `Data_Nascita`,`p`.`ID_Cliente` AS `ID_Cliente` from (`clienti_privati` `p` join `clienti` `c`) where (`p`.`ID_Cliente` = `c`.`ID`) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `aziendali` AS select `c`.`ID` AS `ID`,`a`.`Nome_Cliente` AS `Nome_Cliente`,`a`.`Cognome` AS `Cognome`,`a`.`Data_Nascita` AS `Data_Nascita`,`a`.`P_IVA` AS `P_IVA`,`a`.`ID_Cliente` AS `ID_Cliente` from (`clienti_aziendali` `a` join `clienti` `c`) where (`a`.`ID_Cliente` = `c`.`ID`) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2018-02-09 14:11:41
